...
app.config['MAX_CONTENT_LENGH'] = 2 * 1024 * 1024 # Limite do tamanho de uploads para 2mb
...
def get_db():
    ...
    if 'db' not in g:
        ...
        g.db.row_factory = sqlite3.Row # Permite acessar os dados como dicionário (ex: linha['email'])
    return g.db
...
def extensao_valida(nome_arquivo):
    # Verificiar se a extensão do arquivo enviado é uma das permitidas
...
@app.route('/')
def index():
    # Exibir todos os post públicos na página inicial
    db = get_db()
    posts = db.execute('''
